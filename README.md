...
**4. 연습 기록 저장**
면접 세션, 질문, 답변, 평가 결과를 PostgreSQL에 저장합니다. 같은 질문을 반복 연습하면서 점수 변화를 추적할 수 있는 구조로 설계했고, 현재는 데이터 적재까지 구현되어 있습니다. 조회 API는 추후 작업 예정입니다.

**5. Redis 캐싱**
동일 공고 요청 시 GPT 호출 제거 (응답 속도 개선)
...
세션 하나당 질문 여러 개, 질문 하나당 답변 여러 번 — 같은 질문을 반복 연습하면서 점수 변화를 추적할 수 있는 구조로 설계했습니다. 현재는 데이터 적재까지 구현되어 있고, 조회 API는 추후 작업 예정입니다.
...
### 현재 한계
- **반복 연습 추적 미구현** — DB 구조는 갖춰져 있지만 히스토리 조회 API가 아직 없음
...
#### Redis 캐싱은 서버 로그로 확인했다

동일 공고 URL로 두 번 요청해서 터미널에 `CACHE MISS → CACHE HIT`가 찍히는 걸 직접 확인했습니다. 첫 번째 요청은 크롤링 + GPT 호출이 발생하고, 두 번째부터는 Redis에서 바로 반환됩니다.
